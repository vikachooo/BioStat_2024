...
cleaned_data %>% 
  ...
  summarise(across(everything(), list(n = ~length(.x), 
                                     NA_values = ~sum(is.na(.)), 
                                     ...
                                     IQR = ~IQR(., na.rm = TRUE), 
                                     min = ~min(., na.rm = TRUE),  
                                     max = ~max(., na.rm = TRUE)))) |> #можно было бы сохранить этот лист в переменную и потом преобразовать в функцию
  pivot_longer(cols = -stroke, names_to = "statistic", values_to = "value") #важно привести все переменные в один формат, иначе ошибка
...
rcor_results <- rcorr(as.matrix(cleaned_data
                |> select(where(is.numeric)))) 

rcor <- rcor_results$r
p_cor <- rcor_results$P

p_cor_adj <- matrix(p.adjust(p_cor, method = "bonferroni"),
                    nrow = nrow(p_cor),
                    ncol = ncol(p_cor))



ggcorrplot(rcor, method = "square", type = "upper", tl.col = "black", p.mat = p_cor_adj, sig.level = 0.05)

# сомнительно получилось, без поправки как-то лучше
...
1. Построим логистическую регрессию, так как зависимая переменная категориальная (0,1). Буду использовать backward stepwise метод - сначала включим все переменные, затем будем удалять незначимые (так быстрее).
