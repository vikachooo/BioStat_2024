...
* **06.11.2024** Проблема множественных сравнений
...
* **20.11, 07.12.2024** Проблема множественных сравнений 2, дисперсионный анализ
...
* Оцениваем сложную систему, состоящую из разных частей
  ...
  * Нам важно совместное поведение оценок, а не поведение отдельных оценок в среднем. _"Все или ничего"_
  * Сравните с последовательным соединением элементов в цепи: чтобы сигнал прошел, отработать должен каждый из них

## Пример 4. Шкала SF-36

_Материал из [Википедии](https://ru.wikipedia.org/wiki/SF-36):_
...
Состояние человека описывается положением на восьми шкалах -- нас интересует точность оценки как целого, а не точность отдельных составных -- используем FWER.
...
df3$pvalue.BH %>% hist(col = "cadetblue3", ylim = c(0, 80))
df3$pvalue.Bonf %>% hist(col = "cadetblue3", ylim = c(0, 80))
```


# Часть 5. Итоги и практические рекомендации
...
* Проводить только те тесты, которые действительно нужны/важны и применять поправки к ним
  * Альтернатива: разумная группировка тестов в "семейства" и применение поправок внутри семейств. Например, сгруппировать тесты по группам пациентов
...
* По возможности репортируйте точные значения p-values, а не дихотомизированные в формате p>0.05 / p<0.05. Это позволит исследователям применить поправки по своему усмотрению
* При применении поправок репортируйте и исходные (raw / unadjusted) p-values
