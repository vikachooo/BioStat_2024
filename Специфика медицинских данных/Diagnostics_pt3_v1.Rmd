...
set.seed(42) # для воспроизводимости результатов на разных машинах
...
roc_curve_2 <- roc(response = arteries_data_cleaned$Arterial_occlusion,
                   predictor = sample(1:100, 140, replace = T))
roc_curve_2
```
...
Мы знаем, что Arterial_occlusion = 1 если US_diameter > 0.5 и Arterial_occlusion = 0 если US_diameter <= 0.5

```{r}

roc_curve_3 <- roc(Arterial_occlusion ~ US_diameter, 
                   data = arteries_data_cleaned,
                   ci = T) # добавим доверительные интервалы
...
```
И идея в том, что мы можем взять только те переменные, которые дают наилучший результат в предсказании, и строить логистическую регрессию только на них


# А ещё мы можем улучшать диагностику...

Создать новые переменные из существующих, которые потенциально могут давать лучшие результаты для предсказания

```{r}

roc_curve_4 <- arteries_data %>%
  mutate(MAP = DAP +1/3*(SAP - DAP),
         CVC = (Perf_M / MAP) %>% round(2)) %>%
  roc(Arterial_occlusion ~ CVC, data = .,ci = T)
